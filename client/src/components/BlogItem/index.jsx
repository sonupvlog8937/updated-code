...
const BlogItem = (props) => {
  const blogLink = `/blog/${props?.item?._id}`;
  return (
    <div className="blogItem group">
       <Link to={blogLink} className="imgWrapper block w-full overflow-hidden rounded-md cursor-pointer relative">
        <LazyLoadImage
          alt={"image"}
          effect="blur"
      ...
      <div className="info py-4">
        <h2 className="text-[15px] font-[600] text-black mb-1  lg:mb-3">
          <Link to={blogLink} className="link">{props?.item?.title}</Link>
        </h2>
        ...
        <Link to={blogLink} className="link font-[500] text-[14px] flex items-center gap-1">Read More <IoIosArrowForward /></Link>
